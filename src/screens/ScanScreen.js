...
* */
const CAM_OPTIONS = {
  quality: 1, base64: true, orientation: RNCamera.Constants.Orientation.portrait,
  fixOrientation: true,
...
export default class ScanScreen extends Component {
  ...
  captureColor = async () => {
    if(this.state.isEnabled){ // live view
      ...
      if (this.camera) {
        ...
        await Image.getSize(data.uri, (width, height) => {
          ...
          this.setState({ capImage : {data: data, imageBase64: data.base64, width: width, height: height}});
        }).then(res => {
  ...
  switchFlash = () => {
    ...
    this.setState({ camera : {...camera, flashOn: flashOn , flashMode: flashOps.flashMode } });
  }
      ...
      return (
          <View style={styles.sliderContainer}>
            <View style={styles.sliderText}>
              <Text white normal regular>{this.state.camera.exposure}</Text>
            </View>
            <MultiSlider
                values={[0]}
